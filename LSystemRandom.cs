...
public class LSystemRandom : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        string lsystem = GenerateLSystem(); // generate L-System
        DrawLSystem(lsystem); // draw L-System
    ...
    // generate L-System axiom in string format. for example "F+F-F-F+F"
    string GenerateLSystem()
    {
        ...
        for (int i = 0; i < iterations; i++)
        {
            ...
            {
                // if character is forward, replace it with random rule
                if (c == FORWARD)
                {
        ...
        Debug.LogWarning("RANDOM L-System: " + result);
        return result;
